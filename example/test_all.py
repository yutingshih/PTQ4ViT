...
from utils.save_file import save_csv, save_model


def test_all(
    name,
    cfg_modifier=lambda x: x,
    calib_size=32,
    config_name="PTQ4ViT",
    ...
    save_model(net, f"{dir}/weights/{name}-{config_name.lower()}_{quant_cfg.bit}.pt".lower())
    save_csv(
        pd.DataFrame(
            {
                "model": [name.replace("patch", "")],
                "config": [config_name.lower()],
                "bit": [quant_cfg.bit],
                "calibration_size": [calib_size],
                "calibration_time": [(calib_end_time - calib_start_time) / 60],
                "accuracy": [acc],
            }
        ),
        f"{dir}/results/results.csv",
    )
...
if __name__ == "__main__":
    ...
    if args.multiprocess:
        ...
        for cfg in cfg_list:
            test_all(**cfg, dataset_root=args.dataset_root)
