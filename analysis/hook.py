...
def hook_softmax_gelu(model) -> Tuple[List[torch.Tensor]]:
    softmax_outputs = []
    gelu_outputs = []
...
def get_activations(model, input, layers=None):
    if layers is None:
        layers = (nn.Conv2d, nn.Linear, nnq.Conv2d, nnq.Linear)
...
def get_weights(model, layers=None, quantized_layers=None):
    if layers is None:
